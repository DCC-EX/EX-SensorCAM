...
String VER = "SensorCAM RGB565 320 x 240   Monitor v201 for Processing4";
int BAUD=115200;
...
int xVal=0;
       ...
       savedYrow=Yrow;
       Yrow=aRow-2;
       if(aRow<10) xVal=getNum(6)-2;
       else xVal=getNum(7)-2;  
       sendCmd('x',xVal);
       sendCmd('z',size);
       executeYcmd();     //redraw 10x10 image       
       Wval=savedWval;    //restore settings
       sendCmd('x',Xval);
       sendCmd('z',Zlength);
       Yrow=savedYrow;
}
