...
String VER = "SensorCAM RGB565 320 x 240   Monitor v204 for Processing4";
int BAUD=115200;
...
int xStartPt=0;
       ...
       savedYrow=Yrow;
       Yrow=aRow-2;
       if(aRow<10) xStartPt=getNum(6)-2;
       else xStartPt=getNum(7)-2;  
       sendCmd('x',xStartPt);
       sendCmd('z',size);
  ...
  // Send the keystroke out:
    ...
    switch(ch) {      //for debugging only
      case 0x0E:      // ^N for no echo
        echo=false; whichKey=0; 
        break;
      case 0x05:      // ^E for echo on
        echo= true; whichKey=0; 
        break;
...
boolean myCommand(char cmd,int nch) { //execute primary (capital) commands
  ...
  switch (cmd) {
    ...
    case 'Y':            //get coordinate y, and fill image
      println("*****YYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYY***** start of Y");
...
/***/ //   print("try to get row row_i="); println(row_i);
 ...
 /***/           print("GOOD header Received. dataZlength is "); println(dataZlength);
                 for( int bf=inPtr-8;bf<(inPtr+18);bf++) print(chInBuf(bf));println("");
...
/***/            print("Done row ");print(row_i);print(" for W");print(WidithRequest);print(" Z");print(Zlength);print(" X");print(xStartPt);print(" Y");println(Yrow); //delay(5000);        
/***/   //          print("row_i= ");println(row_i);          // processed header&data & done row image()");
