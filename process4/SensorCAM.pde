...
String VER = "SensorCAM RGB565 320 x 240   Monitor v208 for Processing4";

int comNo=1;  //0 for first COM port. May force another port e.g. =1 for second port.
final int SF= 3;  // image scale factor (2-4) (4 for highest resolution screens) 
int  mirrorX= 1;  //NO mirror if =1, flipped if -1  Flip(H) reverses b/s to s/b colours
int  mirrorY= 1;  //Flip(V) by rows places origin ->bottom corner.  Flip(H) origin ->right
...
void mousePressed(){
      if (newS){          // recalculate coordinates if image flipped
        if (mirrorX<0) 
             coordX = (SF*319+10-mouseX)/SF-1;
        else coordX = (mouseX-10)/SF-1;
        if (mirrorY<0) 
             coordY = (SF*239+30-mouseY)/SF-1;
        else coordY = (mouseY-30)/SF-1;
      } 
}
