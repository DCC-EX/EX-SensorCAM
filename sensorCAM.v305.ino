...
#define BCDver 305
    //v305 more minor code neatening 
    ...
    //v217 mods to 'n' and 'ver' for DCC-EX-CS  suppress "Waiting" if newi2cCmd. '@'/'`'
...
bool    DEBUG[11]={false}; //show detail debug to USB output
...
/*  sensorCAM commands:
a%%   Activate sensor[%%]  & refresh Sensor_ref[%%], cRatios etc. from the [S%%] image (48 bytes) in latest frame.
...
n$[,##] * Number of bank($) assignment to the programmable LED to show its occupancy status. [n$,## minSensors=##]
...
@##   Changes Occupied indicator from 35(#) to any nominated ASCII character from 1 to 127. @12 or BOLD spade
+#,$ *Add to position - camera alignment adjustment. Move ALL sensors #(0-3) pixels in $ direction(0-7)  
...
byte Sensor666[80*16*3]={10}; //buffer to hold decoded latest 4x4 sensor images from frame fb (pitch=S666_row=12)
...
byte SensorBlockStat[11]={0};//array of bits (8/bank) each high if corresponding sensor tripped
          // If SensorBlockStat[b] ==0 then NO sensors in that block set are occupied (sensor[b0] is LSB in byte)
unsigned int quad[5]={0};    //a temporary place to hold current sensor image quadrant brightness values 
...
void loop() {
 //execte once per 100mSeconds (sample rate)
  ...
  static int  rollAvFrameR[64]={0}, rollAvFrameG[64]={0}, rollAvFrameB[64]={0};  //rolling Average values of Sensor[00]
...
void disp_sample_pixels(byte *imagePtr,int pitch,int row,int dispX,int nBytes) //nBytes is #/line (12 per sensor)
 */
...
/***/ IFI2C for(i=0;i<12;i++) Spr(char(cmdString[i]));
    ...
    int     xVal=get_number(&cmdString[i]);
            if(xVal<0 || xVal>319) printf("invalid xValue\n");
              ...
              printf("(a%%%%,rrr,xxx)set new coordinates. Sensor[%d/%d] to r:%d x:%d\n",bsn>>3,bsn&7,rowVal,xVal);                    
        ...
        if (bsn>=0) { printf("(a%%%%) enAble: sensor %d/%d with fresh reference - bsNo 0%o\n",bsn>>3,bsn&7,bsn);         
          ...
          grab_ref(bsn,Sensor666,S666_pitch,long(bsn*S666_pitch),&Sensor_ref[bsn*S666_pitch],
           ...
           //NOTE WELL: without new frames, an new reference won't be much use with new coordinates!
...
/***/       IFN write_img_sample(&Sensor666[bsn*48], S666_row, 0L, 12 );//prints out ref & new img[bsNo] (1 frame)
...
bool processDiff(int bsn){
    ...
    static int  SensorFilter[80];     //use to slow dropout by a cycle or two - needs to be unoccupied for more than 100mSec
    int SenHiCnt;
    int Threshold;
 ...
 //  int numWritten;
       ...
       switch(i2cCmd[0]) {        //first character decides return data      
        ...
        case 'i':            //i%%: //write state of sensor #/#    
          /*numWritten=*/ MyWire.write(dataPkt,10);
          break; 
...
/***/          IFI2C Serial.print(i2cCmdBuf[0],HEX);
 ...
 //     case EXIORDAN:                  //EXIORDAN(0xE4) Read full set analogue values (@20Hz)**duplicate RDD**
        //as alternative to '@'/'`'    //lower case'@' gives '`'    
      case EXIORDD:                   //EXIORDD(0xE6)read ALL digital bits (@100Hz)
