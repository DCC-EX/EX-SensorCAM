...
#define driverVer 301 //203-devel_cam23
// v300 stripped & revised without expander functionality. Needs sensorCAM.h v300 AND CamParser.cpp
// vo322 uses '@'for EXIORDD read.  handles <NB $> and <NN $ ##>
// vo316 includes 'j' command and uses CamParser rather than myFilter.h Incompatible with v203 senorCAM
...
int _configureAnalogIn(VPIN vpin) override { 
  ...
  return true;     // use of EXRAIL IFGTE() etc use "analog" reads.
}
//*************************
...
void _loop(unsigned long currentMicros) override {
    ...
    if (_readState == RDS_IDLE) {    //check if time for digitalRefresh   
      if ( currentMicros - _lastDigitalRead > _digitalRefresh) { 
        ...
        if (currentMicros - _lasttStateRead > _tStateRefresh)  // Delay for "analog" commands
...
int processIncomingPkt(uint8_t *rBuf,uint8_t sensorCmd) {
 ...
 // if (sensorCmd <= '~') DIAG(F("processIncomingPkt %c %d %d %d"),rBuf[0],rBuf[1],rBuf[2],rBuf[3]);
  switch (sensorCmd){
    ...
    case 'm':
      DIAG(F("(m$[,##]) Min/max: $ frames min2flip (trip) %d, maxSensors 0%o, minSensors 0%o, nLED %d, threshold %d, TWOIMAGE_MAXBS 0%o"),rBuf[1],rBuf[3],rBuf[2],rBuf[4],rBuf[5],rBuf[6]);                                                               
      break;
...
void _writeAnalogue(VPIN vpin, int16_t param1, uint8_t camop, uint16_t param3) override {
    ...
    if(camop >= 128) {   //case "a" (4p) also (3p) 
