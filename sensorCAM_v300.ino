...
#define BCDver 300
    ...
    //208 tweak to get 'X' recognised in new format
...
n$[,##] * bank Number $ assignment to the programmable LED to show its occupancy status. [n$,## minSensors=##]
...
/**/         Spr('\n');Spr(i);Spr("bytes: ");Spr(char(i2cIncoming[0]));Spr(' ');Spr(i2cIncoming[1],HEX);Spr(' ');Spr(i2cIncoming[2],HEX);Spr(' ');
/**/     Spr(i2cIncoming[3],HEX);Spr(' ');Spr(i2cIncoming[4],HEX);Spr(' ');Spr(i2cIncoming[5],HEX);Spr(' ');Serial.println(i2cIncoming[6],HEX);       
/**/   }
   ...
   //     MyWire.write(dataPkt,(NUMdigPins+7)>>3);    //preload onRequest buffer      // ######  
       ...
       if(i2cIncoming[0]<='^'){       // then capital cmd instead of EXIOWRAN (0xEA) encoded
         ...
         if((i2cIncoming[0] == 'a')&&(par3 < 1000)) {   //assumes 4 par format
            // par1=(i2cIncoming[1]/10)*8+(i2cIncoming[1]%10 & 0x07);    //bsn back to bsNo
             par1=i2cIncoming[1];
...
void i2cRequest(){                             //function to handle i2c request (interrupt?)
   ...
   int numWritten;
       ...
       if (strchr((const char *)F("Fjolarsuwegx"),i2cCmd[0]) != nullptr) 
