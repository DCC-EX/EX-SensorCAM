...
t##[,%%] Threshold level being used. show/set for Sensors (## sets 32-98)[for S%%] t1 toggle scrolling  <1+ bytes>
    ...
    switch(cmdString[0]) {  
      case 'a':{      //a%%;  enAble Sensor[bsn] and get fresh reference (only do if UNOCCUPIED!)
        ...
        if (bsn>=0) { printf("(a%%%%) enAble: sensor %d/%d with fresh reference - bsNo 0%o\n",bsn>>3,bsn&7,bsn);         
          ...
          pvtThreshold[bsn]=255;   //no pvtThreshold   //new coordinates so cancel pvtThreshold?                                                          
   ...
   for(sn=0;sn<80;sn++){    //decode all 80 Sensor images  
     if(SensorActive[sn]){ //for speed, only active sensors decoded. This function only takes <400uSec for 80
       ...
       if((pvtThreshold[sn]<0x80)||(pvtThreshold[sn]==255)){  //not linear sensor
         ...
         int lstepR =2*((pvtThreshold[sn]&0x7F)>>4); //linear row step Pixels x2
         int lstepX =2*( pvtThreshold[sn] & 0x0F);
         if (lstepX>15) lstepX = -(lstepX&0x0F); //negative step 
...
bool makeLinear( int bsn, int deltaR, int deltaX ) {  //return true if good
  int bsnP1;       //delta arguments must be <= 7 
    if((deltaR>7) | (abs(deltaX)>7)) return false;       //step size too big.
    if(Sensor[bsn]>(235-2*deltaR*3)*FBPITCH){
      deltaR=0; Serial.print('S');Serial.print(bsn,OCT);Serial.println(" line flattened at edge");}         //flatten line if going to step to edge
    ...
    if(bsn%8 != 7) {         //create next sensor
      deltaR = 2*deltaR;     //2 pixels per step
      deltaX = 2*deltaX;
      Sensor[bsnP1] = Sensor[bsn] + FBPITCH*deltaR*4 + deltaX*4*2;  //next sensor 2bytes/pixel (RGB565)
         //check if new sensor within bounds
      if(Sensor[bsnP1] > long(235L*FBPITCH)-90) {       //note: no check on line length wrap around.
        Serial.print(Sensor[bsnP1]),Serial.println(" sensor[bsn+1] on bottom limit");    
        long rowAdjust=(Sensor[bsnP1]/FBPITCH-234)*FBPITCH;    
