...
*
* To create EX-SensorCAM devices, these are defined in myHal.cpp:
* (Note the device driver is included by default)
*
* void halSetup() {
*   // EXSensorCAM::create(vpin, num_vpins, i2c_address);
*   EXSensorCAM::create(700, 91, 0x11);  
* }
* 
...
/***/ DIAG(F("EX-SensorCAM ESP32 driver v1.0 used for addresses up to 0x%x"),ESP32CAP);
...
bool _configure(VPIN vpin, ConfigTypeEnum configType, int paramCount, int params[]) override {
 ...
 // note: if I try this with a real expander second call will cause an error unless differentiated by I2CAddress ESP32CAP!   
...
// ********
//   first packet read from ESP32 is expected to be old EXIORDD so read again 
//   note: if I try this with a Arduino expander second call may cause an error unless differentiated by I2CAddress ESP32CAP!   
 int errors = ioESP32(_I2CAddress, responseBuffer, 1, digitalOutBuffer, 3);  //use only for commands responding with EXIORDY  
     if ((errors==0) && (responseBuffer[0] == EXIORDY) {
// ********     
*/   if (responseBuffer[0] == EXIORDY) {
        return true;
  ...
  void _loop(unsigned long currentMicros) override {
    ...
    if (_readState == RDS_IDLE) {
/***/   int slowCAM = 0; 
 //   if (_I2CAddress<=ESP32CAP) slowCAM=1;           //halve i2c bus usage for sensorCAM EXIORDD-20mSec adequate.
/***/   if (currentMicros - _lastDigitalRead > (_digitalRefresh<<slowCAM)) { // Delay for digital read refresh
...
// ********
// Check if ESP32, and if so, read again and reestablish EXIORDD while trashing surplus junk packet
  int ioESP32(byte i2cAddr,uint8_t *rBuf,int inbytes,uint8_t *outBuff,int outbytes) {
    ...
    //these commands can invoke a return data  packet
...
/***/ //   Serial.print("=");Serial.print(rBuf[0],HEX);Serial.println(inbytes);
	    //  if (sensorCmd != '-')    //try again once  //try again for the sake of the 'k' cmd.
        {                          //try again for pipelined return data
	  	    status = I2CManager.read(i2cAddr, rBuf, inbytes, outBuff, outbytes);
/***/ //    Serial.print(inbytes);Serial.println(char(rBuf[0]));
/***/ //  for(int i=0;i<inbytes;i++) Serial.print(rBuf[i],HEX); Serial.println('%');
        }
        if (rBuf[0] != EXIORDY) {   //perhaps got back ascii cmd
          if (rBuf[0] == sensorCmd){
/***/  //   Serial.println("got match on sensorCmd");
            if(sensorCmd=='i') { int k=256*rBuf[5]+rBuf[4];     //information from i##
...
/***/  //     DIAG(F("(t[##]) Threshold:%d sensor[00]: -%d"),rBuf[1],rBuf[2]&0x7F,rBuf[2]>>7);
              Sp("(t[##[,##]]) Threshold:"); Sp(rBuf[1]);Sp(" sensor S00:"); Sp("-");Sp(rBuf[2]&0x7F);
              if(rBuf[2]>127)Sp("##*\n"); 
              else{ if(rBuf[2]>rBuf[1])Sp("-?*\n"); else Sp("--*\n");}
...
#endif
/***/ //    Serial.print("WriteAnalogue pin ");Serial.print(vpin,DEC);Serial.print(profile,DEC);
...
/***/ //   DIAG(F("WriteAnalogue 0x%x %d %d %d %d"),servoBuffer[0],servoBuffer[1],servoBuffer[2],servoBuffer[3],servoBuffer[4]);
/***/ if(_I2CAddress>ESP32CAP) {
