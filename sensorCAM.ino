...
#endif
#ifndef STEPX_SF
#define STEPX_SF 8      //scaleFactor to adjust linear sensor X steps
#endif
#ifndef STEPR_SF
#define STEPR_SF 8      //scaleFactor to adjust linear sensor R steps
#endif
...
#include <EEPROM.h>
#define EEPROM_SIZE 320+8+80+80 //long pointers into image +reboot flag(int)(+threshold,nLED,min2flip,maxSensors?)
...
//int  averageSensor[48*80]={0};    //accumulation space for up to 64 frames of a sensor 
...
/***/   IFT Spr("\n******** ");
      ...
      timerLoop=millis();
      if(loopTime<100)Spr(' ');
        IFS if(tcounter>0){tcounter--; if(tcounter==1) scroll=false;}
...
/***/ IFd 7) if (SensorBlockStat[h7block] != 0){Spr("debug-trip in Block:");Serial.println(h7block);wait();} 
...
/***/      //             Sensor_ref[bsn*S666_pitch],Sensor_ref[bsn*S666_pitch+1],Sensor_ref[bsn*S666_pitch+2],
/***/      //             Sensor_ref[bsn*S666_pitch+3],Sensor_ref[bsn*S666_pitch+4],Sensor_ref[bsn*S666_pitch+5]);    
        } break;
      }     
      case 'b':{ //b$; Block status byte output to host on USB (& i2c)  //b$# use # to change brightSF from 1 to 4     
        if (cmdString[2]==',') cmdString[2]=cmdString[3];
      ...
      case 'i':{      //i%%;  Individual sensor status byte (1/0) sent to USB (& i2c)
        ...
        if (pvtThreshold[bsn]!=255){ printf(" pvtThreshold= %d ",pvtThreshold[bsn]);
          if(pvtThreshold[bsn]>=128){
            ...
            printf(" Linear sensor length: stepR %d & stepX %d pixels",lstepR*STEPR_SF,lstepX*STEPX_SF);
          }
        }
        b=0;
   ...
   int    xVal=get_number(&cmdString[i]);
      ...
      case 'm':{      //m$;    Minimum: sequential frames to trigger Occupied status (default 2)
        ...
        if (isDigit(cmdString[1])) {   //default value for min2flip = 2
          b = cmdString[1]-0x30;
          if(isDigit(cmdString[2])) b = cmdString[2]-0x30;  //accept m0#
          if(b>0) min2flip = b;                //only allow 1-9        
        }
        param2=99;                 //Optional set minSensors (useful when using ~15+ sensors to limit display)
        if (cmdString[2]==',')  param2=get_number(&cmdString[2]);         //=0 if invalid value
        else if(isDigit(cmdString[2]) && (cmdString[3]==',')) param2=get_number(&cmdString[3]);       
        if(param2!=99){
          if (param2 >=100) minSensors = param2%50;  //limit to 0-49.
          else  if(param2<80) maxSensors = param2;     //limit to 0-79.               
        }
      ...
      case 'n':{      //n;  Number of block to assign to the programmable LED
        if (isDigit(cmdString[1])) {        //should use 'e' command to reinstate after next reboot 
         // nLED = cmdString[1]-0x30;
          int n = get_number(&cmdString[0]);
          if(n<10) {nLED = n;   //accept n0#            
            digitalWrite(pLED,HIGH);          //initial setting (LED off) in case it never gets updated elsewhere.
          }
          param2=99;              //Optional set minSensors (useful when using ~15+ sensors to limit display)
          if (cmdString[2]==',')  param2=get_number(&cmdString[2]);         //=0 if invalid value
          else if(isDigit(cmdString[2]) && (cmdString[3]==',')) param2=get_number(&cmdString[3]); 
          if (param2 <80) minSensors = param2;
        }  
      ...
      case 'p':{      //p#;  Print table of Sensor[] Position Pointers (banks 0 to # only)
        i=0; c=0;          
        if(!isDigit(cmdString[1])) b=3;  //p default value;
        else {
          b=cmdString[1]-0x30;
...
/***/       IFN write_img_sample(&Sensor666[bsn*48], S666_row, 0L, 12 );//print out ref & new img[bsNo] (1 frame)
      ...
      case 's':{      //s%%;  Scan for bright spot and set new xy for Sensor[bsNo]
        bsn = get_bsNo(cmdString);       //s0# or s%#; accepted as Sensor number (bsn=8*%+#). Second digit <=7
...
/***/ //debug      Serial.println(cmdString);
        int param1 = get_number(cmdString);  //no number returns 0
        if((param1==00) && (cmdString[3]==',')) 
          param1=99;  //t0,%% does nothing, t00,%% and t99,%% clears pvtThreshold[%%]
        if(param1 > 30) {                 //OK threshold
          ...
          }else {tcounter=param1+2;scroll=true;}
        }
        if(tcounter==0) printf("(t##) Threshold: trip point set to %d \n", threshold);
        wait();        
        break;
      }
      ...
      case 'R': {     //R;  Reset command reboots the CAM in Sensor mode and restores parameters from EPROM
        if(cmdString[0]!='F'){                          //'F' forces Restart immediately (for EX-CS)
          printf("(R)Reset: Press 'Enter' to confirm reset in Sensor Mode - 'R' or 'aw' will abort Reset & wait");
      ...
      case '/':{       //create a linear sensor bank sloping down left 
        int sign=-1;   //step size of $ x 2 pixel width
        if (cmdString[0] == '\\')sign = +1;    //reverse sign 
        printf("(\\%%%%,#,$) linear sensor creation starting with S%%%%. row,x increments: 2*#,2*$ pixels"
                                                                       "(use '/' for -x)\n");
        if(!isDigit(cmdString[4]) && !isDigit(cmdString[6])) {Serial.println("parameter error"); return;}
        bsn=get_bsNo(cmdString);        //needs better check - use parser!       
        i=bsn%8;
        for(i=0;i<(8-bsn%8);i++){       //do to end of bank  
          if(!makeLinear(bsn+i,int(cmdString[4]-'0'),sign*int(cmdString[6]-'0'))) return; 
        }       
        for(i=bsn;i<=(bsn|7);i++){printf("sensor S%o ",i);
          printf(" r=%3d x=%3d ",int(Sensor[i]/pitch),int((Sensor[i]%pitch)/2));
          if (SensorTwin[i]!=0)printf(" Twin = 0%o ",SensorTwin[i]);
          if (pvtThreshold[i]!=255){ printf(" pvtThreshold= %d ",pvtThreshold[i]);		 
          printf(" \n");
          }
        }printf("completed linear line to S%c7. \n",cmdString[1]);
        wait();
        break;
      }
      ...
      case '+':{      //+#,$  Add/move all sensor alignments +/-# pixels in direction $(0-7)*45deg.
        if(isDigit(cmdString[1]) && (cmdString[1]>'3')) { Serial.print("(+#,$) invalid realign cmd\n"); break; }
        if(isDigit(cmdString[3]) && (cmdString[2]==',')){  //confirm valid command
          param2=cmdString[3]-0x30;     //code assumes no sensor too near edge of image!   
          if (strchr("123",cmdString[3]) != nullptr) c=cmdString[1]-'0';  //column
          if (strchr("345",cmdString[3]) != nullptr) b=cmdString[1]-'0';  //row
          if (strchr("567",cmdString[3]) != nullptr) c=-(cmdString[1]-'0');
          if (strchr("701",cmdString[3]) != nullptr) b=-(cmdString[1]-'0');
          for (bsn=0;bsn<80;bsn++){
            if(Sensor[bsn]!=0) {    //bsn defined
              Sensor[bsn] = Sensor[bsn]+b*pitch + c*2;
              if(bsn==0) printf("(+#,$) move all alignment +/-%c pixels (direction %d). New S00 at r=%d x=%d\n",
                            cmdString[1],param2,int(Sensor[bsn]/pitch),int((Sensor[bsn]%pitch)/2));  
            }             
          }
...
int j;     //destination offset 
   for(sn=0;sn<80;sn++){    //decode all 80 Sensor images  
     if(SensorActive[sn]){ //for speed, only active sensors decoded. This function only takes <400uSec for 80
       ...
       if((pvtThreshold[sn]<0x80)||(pvtThreshold[sn]==255)){  //not linear sensor
         ...
         int lstepR =STEPR_SF*((pvtThreshold[sn]&0x7F)>>4); //linear row * STEPR_SF Pixels/step
         int lstepX =STEPX_SF*( pvtThreshold[sn] & 0x07);
  ...
  //         iPtr=iPtr0+quad*(lstepR*pitch+lstepX*2);  //LHS of start of quadrant image
             iPtr=iPtr0+((quad*lstepR)>>2)*pitch+((quad*lstepX)>>2);  //round down pixel count
           j = sn*S666_pitch + row*S666_row +(quad & 1)*6;          
           for (i=0;i<2;i++) {                   //do for quad of four image pixels
             decode565(iPtr,long(pitch*i) , &f666[j+i*S666_row]);     //do a row pair (L & R)
             decode565(iPtr,long(pitch*i+2),&f666[j+i*S666_row+3]);              
           }
         }
       }    
     }
...
/**/   IFd 8) if((i2cIncoming[0] != '@') && (i2cIncoming[0]<0x80)){
...
/***/        Serial.print(cmds[cmdNo]);   Serial.println(x);
     ...
     /*      if(cmdNo==14) {  //14=254=cmds[14}='m'
             Nbytes = i2cPrepare((char)cmds[14],(char)'-',(char *)datapk[0]);   //make an up-to-date dataPkt 
             MyWire.write(dataPkt,Nbytes);
             return 6;   
           }  
           */
          // if(cmds[cmdNo]=='n') return 6;
...
void refRefresh(bool suspend){ 
  ...
  int i;
    ...
    IFd 9) printf("DEBUG-refRefresh suspend: %d lastsuspend: %d rbsn: %d force_refs: %d \n",suspend,lastsuspend,rbsn,force_refs);  
...
bool makeLinear( int bsn, int deltaR, int deltaX ) {  //return true if good
  int bsnP1;       //delta arguments must be <= 7 
    if((deltaR>7) | (abs(deltaX)>7)) return false;       // size too big.
    if(Sensor[bsn]>(235-((STEPR_SF*deltaR*3)>>2))*FBPITCH){
      deltaR=0; Serial.print('S');Serial.print(bsn,OCT);Serial.println(" line flattened at edge");}         //flatten line if going to  to edge
    pvtThreshold[bsn]=0xFF & (0x80 + (deltaR<<4) + (deltaX & 0x08) + abs(deltaX));      ;   //record offsets
    SensorTwin[bsn]=0;
    SensorActive[bsn]=true;
    SensorActiveBlk[bsn>>3] |= mask[bsn&0x07];   
    bsnP1=bsn+1;
    if(bsn%8 != 7) {         //create next sensor
      deltaR = STEPR_SF*deltaR;     // pixels between S%% and S%%+1 
      deltaX = STEPX_SF*deltaX;
      Sensor[bsnP1] = Sensor[bsn] + FBPITCH*deltaR + deltaX*2;  //next sensor 2bytes/pixel (RGB565)
         //check if new sensor within bounds
      if(Sensor[bsnP1] > long((fbheight-5)*FBPITCH)-90) {       //note: no check on line length wrap around.
        Serial.print(Sensor[bsnP1]),Serial.println(" sensor[bsn+1] on bottom limit");    
        long rowAdjust=(Sensor[bsnP1]/FBPITCH-(fbheight-6))*FBPITCH;    
        Sensor[bsnP1]=(Sensor[bsnP1]-rowAdjust);        //crop row
      }                      //now complete new standard Sensor[bsnP1]
      pvtThreshold[bsnP1]=255;   //no pvtThreshold      
      SensorTwin[bsnP1]=0;  
      SensorActive[bsnP1]=true;
      SensorActiveBlk[bsnP1>>3] |= mask[bsnP1&0x07]; 
      grab_ref(bsnP1,Sensor666,S666_pitch,long(bsnP1*S666_pitch),&Sensor_ref[bsnP1*S666_pitch],
       /* make new sensor legitimate non blank */             &Sen_Brightness_Ref[bsnP1],&SensorRefRatio[bsnP1*12]); 
